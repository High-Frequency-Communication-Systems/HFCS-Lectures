...
\begin{tikzpicture}[scale=2,tdplot_main_coords,cube/.style={very thin,black},
    ...
    \draw[thick,->] (3.3,0,0) -- (4,0,0) node[anchor=south east]{$z$};


    system axes
    \draw[thick,<->] (2,.1,0) -- (3,.1,0) node[anchor=north]{$d$};
    ...
    \foreach \x in {0,...,3}
    \draw[black,fill=red] (\x,0,0) circle (1pt) node[color = black, anchor= east]{$e^{\x \times j  \alpha}$};
