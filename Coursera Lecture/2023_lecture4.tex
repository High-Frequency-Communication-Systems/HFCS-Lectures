...
\begin{frame}
    ...
    \begin{outline}
        \1 Some factors determine the desired radiation pattern
        \2 Array Geometry
        \2 Element Spacing
...
In general, we plot the AF in the normalised form (divided by $N$).
...
\begin{columns}[T] % align columns
    \begin{column}{.7\textwidth}
Some famous antennas such as the \textit{Yagi-Uda} implement this. The \textit{Hansen-Woodyard} end-fire array also does it by introducing an \textcolor{red}{excess phase delay}:
\small
\begin{align*}
