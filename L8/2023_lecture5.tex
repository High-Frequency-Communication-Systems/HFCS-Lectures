...
\begin{frame}
    ...
    Generally, the Array Factor for a 3D array can be described by first expressing the elements in the form of a position vector:
    \small
    \begin{align*}
...
\begin{frame}
    ...
    Considering a $5 \times 5$ planar array with element spacings $d_x = d_y =  \lambda/2$ and the phases $\alpha_x = \alpha_y = -\pi/(2 \sqrt{2})$. The radiation pattern looks like this:
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=.35\textwidth]{3d_polar_plot_3.pdf}
    ...
    \begin{columns}[] % align columns
        \begin{column}{.5\textwidth}
            \begin{outline}
                ...
                \1 We have equal line lengths for each element
                \2 Phase and amplitude are the same across the elements
